...
l <- kmeansocc(xtrn, xtst, k=k, fracrej = 0.05, seed = 1)
L <- l[[1]]
clusterId <- l[[2]]
centroids <- l[[3]]

plot(xtrn[,1],xtrn[,2],pch=19,col=clusterId)
...
for (i in seq_along(R)){
  l <- kmeansocc(xtrn,xval,k=k,fracrej = R[i],seed=1)
  L <- l[[1]]
...
#run optimal model and test
out <- kmeansocc(xtrn, xtst, k=k, fracrej = fracrej_opt, seed=1)
pred <- out[[1]]
...
viz(data = xtrn, resolution = 100, classifier = "kmeans", k = k, fracrej = fracrej_opt, seed = 1)
