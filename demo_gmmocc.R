...
k <- 6 #number of Gaussian components
l <- gmmocc(xtrn, xtst,  k = k, fracrej = 0.05, seed = 1)
L <- l[[1]]
mod <- l[[3]]
centers <- t(mod$parameters$mean)

plot(xtrn[,1],xtrn[,2],col=mod$classification,pch=19, 
     xlab = "Feature 1", ylab = "Feature 2")
points(centers[,1],centers[,2],pch=4,col="magenta",lwd=5)

#Visualize mixture model density plot
...
for (i in seq_along(R)){
  l <- gmmocc(xtrn,xval,k=k,fracrej = R[i],seed=1)
  L <- l[[1]]
...
#run optimal model and test
out <- gmmocc(xtrn, xtst, k=k, fracrej = fracrej_opt, seed=1)
pred <- out[[1]]
...
viz(data = xtrn, resolution = 100, classifier = "gmm", k = k, fracrej = fracrej_opt, seed = 1)
