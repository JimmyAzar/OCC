...
#Obtain optimal h once, to be used later 
#out <- parzenocc(xtrn, xtst, h = seq(0.01,10,length.out=50), fracrej = 0.05)
#hopt <- out[[3]]
hopt <-1 
...
for (i in seq_along(R)){
  l <- parzenocc(xtrn,xval,h=hopt,fracrej = R[i])
  L <- l[[1]]
...
#run optimal model and test
out <- parzenocc(xtrn, xtst, h=hopt, fracrej = fracrej_opt)
pred <- out[[1]]
...
viz(data = xtrn, classifier = "parzen", h = hopt, fracrej = fracrej_opt, resolution = 100)
...
#visualize Parzen density estimation over the 2D scatterplot
vizDensity(data = xtrn, h = hopt, fracrej = fracrej_opt, resolution = 100)
